...
test('all nine logos and four card assets exist with official provenance',()=>{assert.equal(sources.length,18);for(const source of sources){assert.ok(fs.statSync(source.file).size>500);assert.equal(new URL(source.url).protocol,['paris','mcdonalds','papa','kfc','lemoa'].includes(source.id)?'user-provided:':'https:');}});
