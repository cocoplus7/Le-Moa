...
정확한 원본 URL과 파일명은 [asset-sources.json](../mobile/src/asset-sources.json)에 기록했습니다. 파리바게뜨·맥도날드·파파존스·KFC와 르모아 로고는 사용자가 2026-09-25에 제공한 원본입니다. 나머지 5개 브랜드와 SKT·KT·U+one·해피포인트 멤버십은 공식 Google Play 배포 아이콘입니다. 브랜드 멤버십에는 해당 브랜드 앱 아이콘을 재사용합니다. 카드 4종은 현대카드·롯데카드의 상품 이미지와 하나카드 공식 PDF의 스마트라이프 카드 앞면입니다. 브랜드 로고와 카드 이미지는 생성형 모델로 재현하지 않았습니다. 르모아 로고는 사용자의 요청에 따라 내장 이미지 편집으로 내부 심볼과 레터링을 확대했습니다. [편집 기록](LEMOA_ICON_EDIT.md)을 참고하세요.
