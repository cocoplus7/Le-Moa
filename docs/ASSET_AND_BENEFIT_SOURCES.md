...
정확한 원본 URL과 파일명은 [asset-sources.json](../mobile/src/asset-sources.json)에 기록했습니다. 파리바게뜨는 사용자가 2026-09-25에 제공한 PB 로고 원본입니다. 나머지 8개 브랜드와 SKT·KT·U+one·해피포인트 멤버십은 공식 Google Play 배포 아이콘입니다. 브랜드 멤버십에는 해당 브랜드 앱 아이콘을 재사용합니다. 카드 4종은 현대카드·롯데카드의 상품 이미지와 하나카드 공식 PDF의 스마트라이프 카드 앞면입니다. 생성형 모델로 로고나 카드를 재현하지 않았습니다.
